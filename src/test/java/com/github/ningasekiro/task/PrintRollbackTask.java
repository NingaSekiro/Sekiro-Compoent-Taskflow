...
@Slf4j
public class PrintRollbackTask implements Task {
    ...
    @Override
    public void rollback(Node node, Context input) {
        log.info("Thread: {} TaskName: {},NodeName:{} is rollback,processed Node: {}",
                Thread.currentThread().getId(),
                TASK_NAME, node.getId(), input.getAllNodeId());
    }
}