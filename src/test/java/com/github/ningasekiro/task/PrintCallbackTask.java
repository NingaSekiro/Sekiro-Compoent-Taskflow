...
@Slf4j
public class PrintCallbackTask implements Task {
    ...
    @Override
    public void callback(boolean success, Node node, Context context) {
        log.info("Thread: {} TaskName: {},NodeName: {} is callback,processed Node: {}",
                Thread.currentThread().getId(),
                TASK_NAME, node.getId(), context.getAllNodeId());
    }
}